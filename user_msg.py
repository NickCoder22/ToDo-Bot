...
async def cmd_start(message: types.Message, database):
...
async def read_stat_freq(message: types.Message, command: CommandObject, database):
...
async def cmd_get_list(message: types.Message, database):
    chat_id = message.from_user.id
    await bot.send_message(chat_id, "Ваш список дел:")
    await get_list(chat_id, database)

@router.message(Command("get_report"))
async def cmd_get_report(message: types.Message, database):
    chat_id = message.from_user.id
